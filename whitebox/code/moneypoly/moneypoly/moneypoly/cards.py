"""Card definitions and deck utilities for MoneyPoly."""
...
class CardDeck:
    ...
    def __repr__(self):
        if not self.cards:
            return "CardDeck(0 cards, next=end)"
        next_index = self.index % len(self.cards)
        return f"CardDeck({len(self.cards)} cards, next={next_index})"
